...
## Entrada 2026-05-05 16:03

> _Gerado automaticamente por analise_v12.py — não editar manualmente este bloco._
...
O **RR (Risk/Reward)** é o divisor entre ganho médio e perda média por trade: `RR = avg_ganho / avg_perda`.

A **Expectância por trade** resume tudo em um número:

```
Expectância = Win% × AvgGanho - (1 - Win%) × AvgPerda
```

### Por que Win% sozinho engana

| Win% | RR | Expectância | Resultado longo prazo |
|------|----|-------------|----------------------|
| 55% | 0.8 | **-0.03 G** | 🔴 Perde dinheiro — parece bom mas destrói capital |
| 50% | 1.0 | **0.00 G** | ⚫ Empate — não cobre custos operacionais |
| 45% | 2.0 | **+0.10 G** | 🟡 Lucrativo apesar de perder mais que ganhar |
| 40% | 3.0 | **+0.60 G** | 🟢 Muito lucrativo — cada loss paga 3 ganhos |
| 50% | 3.0 | **+1.00 G** | 🏆 Ideal — Alta win + Alto RR |

> **Conclusão**: um robô com Win%=40% e RR=3.0 bate um robô Win%=55% e RR=0.8 no longo prazo. O RR define se a estratégia é matematicamente viável.

### Score de ranking usado neste relatório

```
Score = Expectância × RR × log(n_trades)
```

- `Expectância` = valor esperado por trade em pontos
- `RR` = amplifica configs que vencem com mais folga
- `log(n)` = penaliza configs com poucos trades (baixa confiabilidade estatística)

---

## Top 5 Configurações — 2026-05-05 16:03

> Ranking calculado sobre **V12 com n ≥ 15 trades e PnL positivo**.
> Atualizado a cada execução do script — reflete os CSVs disponíveis no momento.

| # | Robô | TF | Lado | n | Win% | PnL (pts) | AvgG | AvgL | RR | Exp/trade | Score | Por quê usar |
|---|------|----|------|---|------|-----------|----- |------|-----|-----------|-------|--------------|
| 🏆 | FORCA_WDO_V12 | 30m | COMPRA | 21 | 61.9% | +1670 | 181 | 86 | 2.12 | 79.5 | 512.1 | Win% alto (62%), RR excelente (2.1), Exp/trade sólida (80pts) |
| 🥈 | FORCA_WDO_V12 | 30m | VENDA | 33 | 57.6% | +2095 | 163 | 78 | 2.10 | 61.2 | 450.0 | Win% alto (58%), RR excelente (2.1), Exp/trade sólida (61pts) |
| 🥉 | FORCA_WIN_V12 | 15m | COMPRA | 94 | 62.8% | +4962 | 142 | 98 | 1.45 | 52.9 | 348.4 | Win% alto (63%), Exp/trade sólida (53pts) |
| #4 | FORCA_WDO_V12 | 20m | VENDA | 45 | 53.3% | +2055 | 154 | 86 | 1.79 | 41.8 | 284.0 | RR bom (1.8), Exp/trade sólida (42pts) |
| #5 | FORCA_WDO_V12 | 5m | VENDA | 89 | 42.7% | +2415 | 144 | 66 | 2.17 | 23.4 | 228.4 | RR excelente (2.2), Exp/trade sólida (23pts) |

> **Recomendação do dia**: `FORCA_WDO_V12` no `30m` lado `COMPRA` — Score=512.1, Expectância=79.5pts/trade, Win%=61.9%, RR=2.12.
