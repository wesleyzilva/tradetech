...
def load(prefix, tf):
    ...
    for pasta in sorted(Path('.').glob('*/')):
        for f in pasta.glob(f'*{prefix}*_{tf}.csv'):
            try:
                df=pd.read_csv(f,sep=';',decimal=',',encoding='latin1',thousands='.')
                ...
                for c in ['Open','Close','High','Low']: df[c]=pd.to_numeric(df[c],errors='coerce')
                df['Volume']=pd.to_numeric(df['Volume'],errors='coerce').fillna(0)
                frames.append(df[['dt','Open','High','Low','Close','Volume']].dropna())
            except: pass
...
def sim(idxs, close_np, high_np, low_np, f_np, sl, tp):
    ...
    n_df=len(close_np)
    for i in idxs:
        il=f_np[i]>0; e=close_np[i]
        tgt=e+tp if il else e-tp
        stp=e-sl if il else e+sl
        out=None
        end=min(i+16,n_df)
        for j in range(i+1,end):
            h=high_np[j]; l=low_np[j]
            if il:
                if l<=stp: out='L'; break
                if h>=tgt: out='W'; break
            else:
                if h>=stp: out='L'; break
                if l<=tgt: out='W'; break
        if out is None:
...
def tabela(prefix, tf, sl, tp, nome):
    ...
    rng=np.where((df['High'].values-df['Low'].values)>0, df['High'].values-df['Low'].values, 0.0001)
    vm=pd.Series(df['Volume'].values).rolling(20,min_periods=1).mean().clip(lower=1).values
    f_np=np.clip((corpo/rng)*(df['Volume'].values/vm)*100,-100,100)
    fa=np.abs(f_np)
    close_np=df['Close'].values; high_np=df['High'].values; low_np=df['Low'].values
    n_df=len(df)
    ...
    for desc, idxs in configs.items():
        ...
        if wr>=42 and pnl>0: rec='ENTRAR SEMPRE'
        elif pnl>0 and wr>=39: rec='ENTRAR'
        elif pnl>0: rec='AGUARDAR'
        else: rec='EVITAR'
